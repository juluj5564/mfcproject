...
void CDdangDdaMuckgiDlg::strategy1() {
	//점이 8개 남았을 때 선분의 갯수를 세는 함수

	//선분의 갯수는 한 횟수가 지날 때마다 경우의 수가 달라진다.
	//점이 8개일 거니까 배열은 8개까지센다.
...
void CDdangDdaMuckgiDlg::OnBnClickedInput()
{
	...
	if(map[x_point1-1][y_point1-1] == 1 && map[x_point2-1][y_point2-1] == 1){
		...
		if(temp !=4){ //그어진 선이 아니라면
			...
			/*	if (!CMyVector::checkLineCross(inputV1, inputV2, V1, V2)) {
					MessageBox(_T("교차"));
					checkFlag = false;
					break;
